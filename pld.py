...
# Features from finale (34 features, including Label)
FEATURES = [
...
FEATURE_NAMES = FEATURES  # 34 features
...
def compute_features(flow_packets, flow_key):
    ...
    stats.update({
        ...
        ' Label': 0  # Default to 0 (Benign) since sniffer doesn’t predict this
    })
    ...
    try:
        ...
        bwd_lengths = [len(p) for p in bwd_packets] if bwd_packets else [0]  # Fixed typo
...
def packet_callback(packet):
    try:
        ...
        if len(flow_sequences[flow_key]) >= 10:
            ...
            if sequence.shape[1] != 34:
                logging.error(f"Feature mismatch: Expected 34, got {sequence.shape[1]}")
                return

            sequence_df = pd.DataFrame(sequence, columns=FEATURE_NAMES)
            # Scaler only has 27 features; pad with zeros for now (temporary workaround)
            missing_features = [f for f in FEATURE_NAMES if f not in scaler.feature_names_in_]
            for f in missing_features:
                sequence_df[f] = 0  # Pad missing features
            sequence_scaled = scaler.transform(sequence_df[scaler.feature_names_in_]).reshape(1, 10, 27)
            # Pad to 34 features with zeros (temporary until scaler retrained)
            sequence_scaled_padded = np.pad(sequence_scaled, ((0, 0), (0, 0), (0, 7)), mode='constant', constant_values=0)
            sequence_scaled_padded = sequence_scaled_padded.reshape(1, 10, 34)

            prediction = model.predict(sequence_scaled_padded, verbose=0)
