...
from .models import BannedIP, BanLog
from .forms import BanIPForm, UnbanIPForm
...
def dashboard(request):
    ...
    recent_logs = BanLog.objects.order_by('-timestamp')[:10]
    # Simple analytics: bans per day
    bans_per_day = BanLog.objects.filter(action='ban') \
    ...
    context = {
        ...
        'bans_per_day': bans_per_day,
    }
    return render(request, 'ecom/dashboard.html', context)
...
def ban_ip(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            BanLog.objects.create(action='ban', ip_address=ip, reason=reason, performed_by=request.user)
            return redirect('dashboard')
    else:
        form = BanIPForm()
    return render(request, 'ecom/ban_ip.html', {'form': form})

@login_required
def unban_ip(request, ip_id):
    ip = BannedIP.objects.get(id=ip_id)
    if request.method == 'POST':
        form = UnbanIPForm(request.POST)
        if form.is_valid():
            reason = form.cleaned_data['reason']
            BanLog.objects.create(action='unban', ip_address=ip.ip_address, reason=reason, performed_by=request.user)
            ip.delete()
            return redirect('dashboard')
    else:
        form = UnbanIPForm()
    return render(request, 'ecom/unban_ip.html', {'form': form, 'ip': ip})
